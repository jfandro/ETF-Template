var universeBubbles = function (divname) {
    ...
    this.colors = d3.scaleOrdinal().domain([0, 100]).range(['#9cbcd7', '#96ddce', '#addd96', '#e6e474', '#e6a074', '#df9393', '#f80707']);
    ...
    this.loadRoot = function () {
        ...
        // Add Y axis (vol)
        me.y = d3.scaleLinear()
            .domain([0, d3.max(data, function (d) { return +d.y + 5; })])
            .range([height, 0]);
        ...
        me.dots
            ...
            .style("fill", function (d) { return me.colors(d.z); })
            .style("opacity", "0.7")
            .attr("stroke", "white")
